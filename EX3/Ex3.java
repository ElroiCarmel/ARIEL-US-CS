...
public class Ex3 {
	...
	private static Point2D prePoint;
	...
	public static void mouseClicked(Point2D p) {
		...
		if (_mode.equals("ShortestPath_p2")) {
			Point2D[] path = _map.shortestPath(prePoint, p);
			if (path!=null) {
				for (int i=0;i<path.length;i++) {
					_map.setPixel(path[i], col);
				}
			}
			_mode = "none";
		}
		if (_mode.equals("ShortestPath")) {
			prePoint = new Point2D(p);
			_mode = "ShortestPath_p2";
		}
		if (_mode.equals("Segment_p2")) {
			_map.drawSegment(prePoint, p, col);
			_mode = "none";
		}
		...
		if (_mode.equals("Circle_p2")) {
			double rad = prePoint.distance(p);
			_map.drawCircle(prePoint, rad, col);
			_mode = "none";
		}
		...
		if (_mode.equals("Rect_p2")) {
			_map.drawRect(prePoint, p, col);
			_mode = "none";
		}
